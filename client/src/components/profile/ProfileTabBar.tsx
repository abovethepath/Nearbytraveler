...
export function ProfileTabBar(props: ProfileTabBarProps) {
  ...
  const ghostLight = isHero && isDesktopWeb && !isOwnProfile;

  const tabBase = "text-sm sm:text-base font-semibold px-3 py-2 rounded-lg transition-all";
  const tabActive = isHero
    ? (ghostLight
      ? "bg-[#e8eeff] text-blue-800 border border-blue-200 shadow-sm dark:bg-white dark:text-gray-900 dark:border-white dark:shadow-md"
      : "bg-white text-gray-900 border border-white shadow-md")
    : "bg-blue-600 text-white border border-blue-600 shadow-md";
  const tabInactive = isHero
    ? (ghostLight
      ? "bg-white/55 border border-blue-100 text-blue-800 hover:bg-white/70 hover:border-blue-200 backdrop-blur-sm dark:bg-white/20 dark:border-white/40 dark:text-white dark:hover:bg-white/30 dark:hover:border-white/50"
