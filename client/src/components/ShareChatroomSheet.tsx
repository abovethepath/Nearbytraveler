...
export default function ShareChatroomSheet({
  ...
  const shareWhatsApp = async () => {
    ...
    if (!url) { try { w?.close(); } catch {} return; }
    if (w) { w.location.href = `https://wa.me/?text=${encodeURIComponent(shareText + "\n" + url)}`; }
  ...
  const shareTelegram = async () => {
    ...
    const url = await generateLink();
    if (!url) { try { w?.close(); } catch {} return; }
    if (w) { w.location.href = `https://t.me/share/url?url=${encodeURIComponent(url)}&text=${encodeURIComponent(shareText)}`; }
    else { window.open(`https://t.me/share/url?url=${encodeURIComponent(url)}&text=${encodeURIComponent(shareText)}`, "_blank", "noopener"); }
  };

  const copyLink = async () => {
    const url = inviteUrl || await generateLink();
    if (!url) return;
    try {
          ...
          {loading ? (
            ...
            <>
              <button
                ...
                className="w-full flex items-center gap-3 px-4 py-3 rounded-xl bg-[#25D366] hover:bg-[#1ebe59] active:bg-[#1aad4d] text-white font-semibold transition-colors cursor-pointer"
              >
              ...
              <button
                ...
                className="w-full flex items-center gap-3 px-4 py-3 rounded-xl bg-[#0088cc] hover:bg-[#006faa] active:bg-[#005a8c] text-white font-semibold transition-colors cursor-pointer"
              >
                <svg className="w-6 h-6 shrink-0" viewBox="0 0 24 24" fill="currentColor">
              ...
              <button
                type="button"
                onClick={copyLink}
                className="w-full flex items-center gap-3 px-4 py-3 rounded-xl bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 active:bg-gray-300 dark:active:bg-gray-600 text-gray-900 dark:text-white font-semibold transition-colors cursor-pointer"
              >
                {copied ? (
