...
export default function DiscoverPage() {
  ...
  const pageContent = (
    <div className="min-h-screen bg-white dark:bg-gray-900 w-full max-w-full overflow-x-clip">
      ...
      {isHeroVisible && !isMobile && !isNative && (
        <section className="relative py-6 sm:py-8 lg:py-3 overflow-hidden bg-white dark:bg-gray-900">
            ...
            <div className="flex justify-center">
              {sortedCities.filter(city => city.city === 'Los Angeles Metro').map((city, index) => (
                <Card
                  ...
                  className="group cursor-pointer hover:shadow-2xl transition-all duration-500 overflow-hidden bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-xl w-full max-w-md rounded-3xl"
          ...
          <div className={`grid grid-cols-2 gap-3 ${isNative ? '' : 'sm:gap-6 lg:grid-cols-3 lg:gap-8'}`}>
            {sortedCities.filter(city => city.city !== 'Los Angeles Metro').map((city, index) => (
              <Card
                ...
                className="group cursor-pointer hover:shadow-xl transition-all duration-300 overflow-hidden bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-lg rounded-2xl"
                onClick={() => setLocation(`/city/${encodeURIComponent(city.city)}`)}
              >
