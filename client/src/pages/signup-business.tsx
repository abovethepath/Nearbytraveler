...
export default function SignupBusinessSimple() {
  ...
  const signupMutation = useMutation({
    mutationFn: async (data: BusinessSignupData) => {
      ...
      // Use businessName as ownerName for database storage
      (processedData as any).ownerName = data.businessName;
    ...
    setTimeout(async () => {
      try {
        ...
        if (response.ok && result.user) {
          ...
          try {
            const profileResponse = await fetch(`${getApiBaseUrl()}/api/auth/complete-profile`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} noValidate className="space-y-4 sm:space-y-6 md:space-y-8 overflow-hidden break-words">
                ...
                <div className="space-y-3 sm:space-y-4 overflow-hidden break-words">
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem className="overflow-hidden break-words">
                        <FormLabel className="text-sm sm:text-base break-words overflow-hidden">Business Email (if different from owner)</FormLabel>
                        <FormControl>
                          <Input 
