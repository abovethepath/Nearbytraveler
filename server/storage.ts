...
export class DatabaseStorage implements IStorage {
  ...
  async assignUserToChatrooms(user: User): Promise<void> {
    ...
    // 2. HOMETOWN CHATROOMS: EVERYONE gets their hometown chatroom (locals, businesses, AND travelers)
    ...
    // 3. TRAVEL DESTINATION CHATROOM: Travelers ALSO get their destination chatroom (in addition to hometown)
    ...
    try {
      ...
      const result = await db.execute(sql`
        SELECT 
          ...
          mc.id as chatroom_id
        FROM quick_meetups qm
        LEFT JOIN users u ON qm.organizer_id = u.id
        LEFT JOIN meetup_chatrooms mc ON mc.meetup_id = qm.id AND mc.is_active = true
  ...
  async getQuickMeetup(id: number): Promise<any> {
    try {
      // Use getQuickMeetupById - it uses raw SQL and correctly maps schema (availableAt/expiresAt, not date/endDate)
  ...
  async joinQuickMeetup(meetupId: number, userId: number): Promise<any> {
    try {
      ...
      // Update participant count in quick_meetups table
      await db
        .update(quickMeetups)
        .set({
      ...
      // Update participant count in meetup_chatrooms table
      await db
