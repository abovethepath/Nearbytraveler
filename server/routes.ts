...
export async function registerRoutes(app: Express, httpServer?: Server): Promise<Server> {
  ...
  app.post("/api/auth/login", async (req, res) => {
    try {
      ...
      console.log("🔍 Password comparison - received:", JSON.stringify(password), "length:", password.length);
      console.log("🔍 Password comparison - stored:", JSON.stringify(user.password), "length:", user.password?.length);
    ...
    try {
      ...
      if (false) { // This block is now disabled - travel plans are created during fast registration
        ...
        try {
          ...
          // REMOVED: Duplicate travel plan creation - already handled in fast registration
          // Travel plan creation and status updates are handled in the fast registration section above
