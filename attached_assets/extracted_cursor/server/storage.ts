...
export interface IStorage {
  ...
  // Travel personality assessment methods
  createTravelPersonalityProfile(profile: any): Promise<any>;
  getTravelPersonalityProfile(userId: number): Promise<any>;
  updateTravelPersonalityProfile(userId: number, updates: any): Promise<any>;
  getCompatibleTravelersbyPersonality(userId: number): Promise<any>;
...
export class DatabaseStorage implements IStorage {
    ...
    const getMetropolitanArea = (city: string, state: string, country: string) => {
      ...
      if (isUS && (normalizedState.includes('california') || stateLower === 'ca')) {
        if (cityLower.includes('playa') || cityLower.includes('santa monica') || 
          ...
          return { city: 'Los Angeles', state: 'California', country: 'United States' };
        }
      }
  ...
  async searchUsersByLocation(location: string, userType?: string, startDate?: string, endDate?: string): Promise<User[]> {
    try {
      ...
      const hometownLocals = await db
        ...
        .where(
          and(
            eq(users.hometownCity, searchCity),
  ...
  async searchUsersByLocationDirect(location: string, userType?: string): Promise<User[]> {
    try {
      ...
      // Handle NYC aliases - search for both Manhattan and New York when someone searches for "New York City"
  ...
  async getUserPhotos(userId: number): Promise<any[]> {
    try {
      console.log('DatabaseStorage.getUserPhotos - Getting photos for user ID:', userId);
      ...
      console.log('DatabaseStorage.getUserPhotos - Found photos:', photos.length, 'for user', userId);
      console.log('DatabaseStorage.getUserPhotos - Raw photo data:', photos.map(p => ({id: p.id, caption: p.caption, userId: p.userId})));
  ...
  async getUserPhotoAlbums(userId: number): Promise<PhotoAlbum[]> {
    try {
      console.log('📸 STORAGE: Getting photo albums for user', userId);
      ...
      console.log(`📸 STORAGE: Found ${albums.length} photo albums for user ${userId}`);
      return albums;
    } catch (error) {
  ...
  async createPhotoAlbum(albumData: InsertPhotoAlbum): Promise<PhotoAlbum> {
    try {
      console.log('📸 STORAGE: Creating photo album:', albumData);
      ...
      console.log('✅ STORAGE: Photo album created:', album);
      return album;
    } catch (error) {
      console.error('❌ STORAGE: Error creating photo album:', error);
      throw error;
    }
  }
  ...
  async getSecretLocalExperiencesByCity(city: string, state: string | null, country: string | null): Promise<any[]> {
    try {
      ...
      // SPECIAL CASE: Los Angeles metro consolidation
      if (city.toLowerCase() === 'los angeles') {
        ...
        // Search for users from ANY LA metro city
        const laConditions = or(
          ...laMetroCities.map(cityName => eq(users.hometownCity, cityName))
        );
        conditions.push(laConditions);
        console.log(`🌍 SECRET ACTIVITIES: Searching ALL LA metro cities for ${city}`);
        
      } else {
        // Generic matching for other cities
        const normalizedCity = city.toLowerCase();
        const normalizedCountry = country ? country.toLowerCase() : '';
        
        if (normalizedCity === 'antwerp' && normalizedCountry.includes('belgium')) {
          conditions.push(ilike(users.hometownCity, '%antwerp%'));
          conditions.push(ilike(users.hometownCountry, '%belgium%'));
        } else if (normalizedCity === 'augusta' && normalizedCountry.includes('united states')) {
          conditions.push(ilike(users.hometownCity, '%augusta%'));
          conditions.push(ilike(users.hometownCountry, '%united states%'));
        } else {
          conditions.push(ilike(users.hometownCity, `%${city}%`));
          if (country && country.toLowerCase() === 'pennsylvania') {
            conditions.push(ilike(users.hometownState, `%pennsylvania%`));
          } else if (country && country.toLowerCase() !== 'united states') {
            conditions.push(ilike(users.hometownCountry, `%${country}%`));
          }
        }
      }

      // Get users with secret activities from this city - ONLY LOCALS FROM THIS CITY
      const whereConditions = [
        ...conditions,
        eq(users.userType, 'local'), // ONLY LOCAL USERS (not travelers, not businesses)
        isNotNull(users.secretActivities), 
        ne(users.secretActivities, '') // ONLY secretActivities, NOT bio
      ];

      console.log(`🔍 SECRET ACTIVITIES: Query conditions for ${city}:`, whereConditions.length, 'conditions');
      
      // Debug: Log the actual conditions being used
      if (city.toLowerCase() === 'los angeles') {
        console.log('🔍 SECRET ACTIVITIES: LA Metro cities being searched:', whereConditions[0]);
      }
      ...
      console.log('Found users with secrets:', usersWithSecrets.length);

      // Create experiences directly from user secretActivities - no separate table needed
      const experiences = [];
      for (const user of usersWithSecrets) {
        // Triple check: Only local users whose HOMETOWN matches this city
        if (user.userType !== 'local') continue;
        
        // For LA metro searches, all LA metro users are valid
        if (city.toLowerCase() === 'los angeles') {
          // Already filtered by LA metro cities in the query, so all users are valid
          console.log(`✅ SECRET ACTIVITIES: Including ${user.username} from ${user.hometownCity} for Los Angeles search`);
        } else {
          // For other cities, verify hometown match
          const userHometown = `${user.hometownCity}, ${user.hometownState}, ${user.hometownCountry}`.toLowerCase();
          const searchLocation = `${city}, ${state || ''}, ${country || ''}`.toLowerCase();
          
          if (!userHometown.includes(city.toLowerCase())) continue;
        }
        
        // Use ONLY secretActivities field, NOT bio
        const secretContent = user.secretActivities?.trim() || '';
        if (!secretContent || secretContent.length < 5) continue;

        // Get actual like count from database
        const likeCount = await db.select({ count: count() })
          .from(secretLocalExperienceLikes)
          .where(eq(secretLocalExperienceLikes.experienceId, user.id));

        const totalLikes = (likeCount[0]?.count || 0) + 1; // +1 for author's automatic like

        // Create experience object directly from user data
        experiences.push({
          id: user.id, // Use user ID as experience ID
          experience: secretContent,
          description: secretContent,
          category: 'local_secret',
          likes: totalLikes, // Actual like count from database
          createdAt: user.createdAt,
          username: user.username, // Direct username field for frontend
          contributor: {
            id: user.id,
            username: user.username,
            name: user.name
      ...
      console.log('Returning experiences:', experiences.length);
      return experiences;
    } catch (error) {
  ...
  async getUsersByLocationAndType(city: string, state: string | null, country: string | null, userType: string): Promise<any[]> {
    try {
      ...
      // Use the fixed searchUsersByLocation logic for consistent location matching
      const locationString = [city, state, country].filter(Boolean).join(', ');
      console.log(`getUsersByLocationAndType - formatted location: "${locationString}"`);
      
      // Get all users using the strict location matching from searchUsersByLocation
      const allMatchedUsers = await this.searchUsersByLocation(locationString, undefined);
      console.log(`getUsersByLocationAndType - searchUsersByLocation returned ${allMatchedUsers.length} users`);
      
      // Filter by user type if specified
      let filteredUsers = allMatchedUsers;
      if (userType && userType !== 'all') {
        if (userType === 'locals') {
          // For "locals" searches, show users who are locals in this city (regardless of travel status)
          filteredUsers = allMatchedUsers.filter(user => user.userType === 'local');
        } else if (userType === 'travelers') {
          // For "travelers" searches, show users who are currently traveling to this city
          const activeTravelPlans = await db
            .select({
              userId: travelPlans.userId,
              destination: travelPlans.destination
            ...
            .from(travelPlans)
            .where(and(
              sql`${travelPlans.startDate} <= CURRENT_DATE`,
              sql`${travelPlans.endDate} >= CURRENT_DATE`
            ));
          
          const travelerUserIds = activeTravelPlans.map(plan => plan.userId);
          filteredUsers = allMatchedUsers.filter(user => travelerUserIds.includes(user.id));
        } else if (userType === 'businesses') {
          filteredUsers = allMatchedUsers.filter(user => user.userType === 'business');
        }
      }
      
      console.log(`getUsersByLocationAndType - after filtering by userType "${userType}": ${filteredUsers.length} users`);
      console.log('getUsersByLocationAndType - final results:', filteredUsers.map(u => ({ 
        id: u.id, 
        username: u.username, 
        userType: u.userType,
  ...
  // Travel Personality Assessment Methods
  async createTravelPersonalityProfile(profile: InsertTravelPersonalityProfile): Promise<TravelPersonalityProfile> {
    try {
      const [newProfile] = await db.insert(travelPersonalityProfiles).values(profile).returning();
      return newProfile;
    } catch (error) {
  ...
  async getTravelPersonalityProfile(userId: number): Promise<TravelPersonalityProfile | undefined> {
    try {
      const [profile] = await db
        .select()
        .from(travelPersonalityProfiles)
        .where(eq(travelPersonalityProfiles.userId, userId))
        .limit(1);
      return profile;
    } catch (error) {
      console.error('Error getting travel personality profile:', error);
      return undefined;
    }
  }

  async updateTravelPersonalityProfile(userId: number, updates: Partial<TravelPersonalityProfile>): Promise<TravelPersonalityProfile | undefined> {
    try {
      const [updatedProfile] = await db
        .update(travelPersonalityProfiles)
        .set({ ...updates, updatedAt: new Date() })
        .where(eq(travelPersonalityProfiles.userId, userId))
        .returning();
      return updatedProfile;
    } catch (error) {
      console.error('Error updating travel personality profile:', error);
      return undefined;
    }
  }

  async getCompatibleTravelersbyPersonality(userId: number): Promise<any[]> {
    try {
      // Get the current user's personality profile
      const userProfile = await this.getTravelPersonalityProfile(userId);
      if (!userProfile) {
        return [];
      }

      // Find other users with personality profiles
      const otherProfiles = await db
        .select({
          profile: travelPersonalityProfiles,
          user: {
            id: users.id,
            username: users.username,
            name: users.name,
        ...
        .from(travelPersonalityProfiles)
        .leftJoin(users, eq(travelPersonalityProfiles.userId, users.id))
        .where(ne(travelPersonalityProfiles.userId, userId));

      // Calculate compatibility scores
      const compatibleTravelers = otherProfiles.map(({ profile, user }) => {
        let compatibilityScore = 0;

        // Adventure Level compatibility (0-20 points)
        const adventureDiff = Math.abs(userProfile.adventureLevel - profile.adventureLevel);
        compatibilityScore += Math.max(0, 20 - (adventureDiff * 5));

        // Social Style compatibility (0-20 points) 
        const socialDiff = Math.abs(userProfile.socialStyle - profile.socialStyle);
        compatibilityScore += Math.max(0, 20 - (socialDiff * 5));

        // Planning Approach compatibility (0-15 points)
        const planningDiff = Math.abs(userProfile.planningApproach - profile.planningApproach);
        compatibilityScore += Math.max(0, 15 - (planningDiff * 4));

        // Budget Consciousness compatibility (0-15 points)
        const budgetDiff = Math.abs(userProfile.budgetConsciousness - profile.budgetConsciousness);
        compatibilityScore += Math.max(0, 15 - (budgetDiff * 4));

        // Cultural Openness compatibility (0-10 points)
        const culturalDiff = Math.abs(userProfile.culturalOpenness - profile.culturalOpenness);
        compatibilityScore += Math.max(0, 10 - (culturalDiff * 3));
        ...
        return {
          ...
          personalityMatch: {
            ...
            planningApproach: profile.planningApproach,
            budgetConsciousness: profile.budgetConsciousness,
            culturalOpenness: profile.culturalOpenness
          }
        };
      });

      // Sort by compatibility score and return top matches
      return compatibleTravelers
        .filter(traveler => traveler.compatibilityScore > 30)
        .sort((a, b) => b.compatibilityScore - a.compatibilityScore)
        .slice(0, 20);
    } catch (error) {
      console.error('Error finding compatible travelers by personality:', error);
      return [];
    }
  }
