import { pgTable, text, serial, integer, boolean, timestamp, real, varchar, jsonb, unique, bigint } from "drizzle-orm/pg-core";
...
export const photoAlbums = pgTable("photo_albums", {
  ...
  title: text("title").notNull(),
  description: text("description"),
  date: text("date").notNull(), // Date of the album/trip
  location: text("location"),
...
// Travel Personality Assessment Table
export const travelPersonalityProfiles = pgTable("travel_personality_profiles", {
  id: serial("id").primaryKey(),
  userId: integer("user_id").notNull().references(() => users.id).unique(),
  travelStyle: text("travel_style").notNull(), // 'adventure', 'cultural', 'relaxation', 'social', 'business'
  budgetLevel: text("budget_level").notNull(), // 'budget', 'mid-range', 'luxury', 'flexible'
  groupPreference: text("group_preference").notNull(), // 'solo', 'small-group', 'large-group', 'flexible'
  communicationStyle: text("communication_style").notNull(), // 'frequent', 'moderate', 'minimal'
  safetyComfort: text("safety_comfort").notNull(), // 'cautious', 'moderate', 'adventurous'
  activityIntensity: text("activity_intensity").notNull(), // 'low', 'moderate', 'high'
  culturalInterest: text("cultural_interest").notNull(), // 'low', 'moderate', 'high'
  socialPreference: text("social_preference").notNull(), // 'introverted', 'balanced', 'extroverted'
  planningStyle: text("planning_style").notNull(), // 'detailed', 'moderate', 'spontaneous'
  accommodationStyle: text("accommodation_style").notNull(), // 'shared', 'separate', 'flexible'
  dateFlexibility: text("date_flexibility").notNull().default('moderate'), // 'rigid', 'moderate', 'flexible'
  personalityType: text("personality_type").notNull(), // Calculated personality type
  compatibilityScore: integer("compatibility_score").notNull().default(50), // Base compatibility score
  assessmentAnswers: jsonb("assessment_answers"), // Raw answers for future analysis
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});

// Travel Personality Profiles schema
export const insertTravelPersonalityProfileSchema = createInsertSchema(travelPersonalityProfiles).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

// Travel Personality Profiles types
export type TravelPersonalityProfile = typeof travelPersonalityProfiles.$inferSelect;
export type InsertTravelPersonalityProfile = z.infer<typeof insertTravelPersonalityProfileSchema>;
...
export const businessOffers = pgTable("business_offers", {
  ...
  discountType: text("discount_type").notNull(), // 'percentage', 'fixed_amount', 'buy_one_get_one', 'free_service'
