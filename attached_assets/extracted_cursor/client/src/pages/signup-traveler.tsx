...
export default function SignupTraveler() {
  ...
  const [formData, setFormData] = useState({
    ...
    // Hometown Location fields
    hometownCountry: "",
    hometownCity: "",
    hometownState: "",

    // Current Travel Location fields
    currentTravelCountry: "",
    currentTravelCity: "",
    currentTravelState: "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!formData.currentTravelCity) missingFields.push("Current Travel City");
      if (!formData.currentTravelCountry) missingFields.push("Current Travel Country");
      if (!formData.travelEndDate) missingFields.push("Travel End Date");
      ...
      const registrationData = {
        ...
        hometownLocation: formData.hometownState
          ? `${formData.hometownCity}, ${formData.hometownState}, ${formData.hometownCountry}`
          : `${formData.hometownCity}, ${formData.hometownCountry}`,
        currentTravelCountry: formData.currentTravelCountry,
        currentTravelCity: formData.currentTravelCity,
        currentTravelState: formData.currentTravelState,
        currentTravelLocation: formData.currentTravelState
          ? `${formData.currentTravelCity}, ${formData.currentTravelState}, ${formData.currentTravelCountry}`
          : `${formData.currentTravelCity}, ${formData.currentTravelCountry}`,
      ...
      if (response.ok && data.user) {
        ...
        // Redirect to welcome, then user can enter profile
        setTimeout(() => {
          setLocation('/welcome');
        }, 1000);
      } else {
          ...
          <CardHeader className="text-center bg-gray-50 dark:bg-gray-800 rounded-t-lg pb-4 sm:pb-8">
            <div className="flex justify-start mb-4">
              <Button
                ...
                className="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-200 border-blue-300 hover:border-blue-500 font-medium px-3 py-2 sm:px-4 sm:py-2"
              >
                <ArrowLeft className="w-4 h-4 mr-1 sm:mr-2" />
                <span className="hidden sm:inline">Back</span>
              </Button>
            </div>
            <CardTitle className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 dark:text-white mb-2 sm:mb-3">
              Complete Your Traveler Profile 🌍
            </CardTitle>
            <CardDescription className="text-base sm:text-lg md:text-xl text-gray-700 dark:text-gray-300 font-medium px-2 sm:px-0">
            ...
            <form onSubmit={handleSubmit} className="space-y-6 sm:space-y-8">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 gap-4">
                  <div>
                    <Label className="text-gray-900 dark:text-white text-sm sm:text-base">Date of Birth *</Label>
                    <div className="text-xs sm:text-sm text-blue-600 dark:text-blue-400 mb-2">
                  ...
                  <div>
                    <Label className="text-gray-900 dark:text-white text-sm sm:text-base">Gender *</Label>
              ...
              <div className="space-y-4">
                <h3 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">Hometown Location</h3>

                <div>
                  <Label className="text-gray-900 dark:text-white text-sm sm:text-base">Hometown Location *</Label>
                  <div className="text-xs sm:text-sm text-blue-600 dark:text-blue-400 mb-2">
                    This is where you're from and will always be seen as a local
                  </div>
                  <SmartLocationInput
              ...
              {/* Current Travel Location Information */}
              <div className="space-y-4">
                <h3 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">Current Travel Location</h3>

                <div>
                  <Label className="text-gray-900 dark:text-white text-sm sm:text-base">Where are you currently traveling? *</Label>
                  <div className="text-xs sm:text-sm text-blue-600 dark:text-blue-400 mb-2">
                    This is where you are now and will be visible to locals and travelers
                  </div>
                  <SmartLocationInput
                    country={formData.currentTravelCountry}
                    city={formData.currentTravelCity}
                    state={formData.currentTravelState}
                    onLocationChange={(location) => {
                      setFormData(prev => ({
                        ...prev,
                        currentTravelCountry: location.country,
                        currentTravelCity: location.city,
                        currentTravelState: location.state
                      }));
                    }}
                    placeholder={{ country: "Select current travel country", city: "Select current travel city", state: "Select state/region" }}
                    required
                  />
                </div>

                <div>
                  <Label className="text-gray-900 dark:text-white text-sm sm:text-base">Travel End Date *</Label>
                  <div className="text-xs sm:text-sm text-blue-600 dark:text-blue-400 mb-2">
                    When does your trip end? (This helps with matching and planning)
                  </div>
                  <Input
                    type="date"
                    ...
                    required
                    className="border-2 border-gray-300 dark:border-gray-600 h-10 sm:h-10"
                  />
                </div>
              </div>
              ...
              <div className="space-y-6">
                <div className="text-center mb-6">
                  ...
                  <div className="text-xs sm:text-sm text-blue-600 bg-blue-50 border border-blue-400 rounded-md p-3 mb-4 dark:bg-blue-900/20 dark:border-blue-500 dark:text-blue-300">
                    <strong>Choose at least 2 choices from the list below, but the more you choose the better to match...</strong>
                  </div>
                  <div className="text-center mt-4">
                    <div className={`text-base sm:text-lg font-bold ${getTotalSelections() >= 2 ? 'text-green-600' : 'text-red-600'}`}>
