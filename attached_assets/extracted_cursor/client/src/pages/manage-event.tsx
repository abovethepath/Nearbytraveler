...
export default function ManageEvent({ eventId }: ManageEventProps) {
  ...
  React.useEffect(() => {
    if (event) {
      ...
      const startDateFormatted = event.date ? new Date(event.date).toISOString().slice(0, 16) : "";
      const endDateFormatted = event.endDate ? new Date(event.endDate).toISOString().slice(0, 16) : "";
      ...
      setFormData({
        ...
        city: event.city || "",
        state: event.state || "",
        country: "", // Events schema doesn't have country field
    ...
    onSuccess: async (imageData: string) => {
      ...
      if (event?.id) {
        try {
          ...
          toast({
            title: "Upload Failed",
            description: `Failed to save image to event: ${error.message || error}`,
            variant: "destructive",
          });
        }
      } else {
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-4">
                <div>
                  <Label htmlFor="street">Street Address/Venue Name *</Label>
                  <Input
                    id="street"
                    value={formData.street}
                    onChange={(e) => setFormData(prev => ({ ...prev, street: e.target.value }))}
                    placeholder="Enter venue name or street address"
                    required
                  />
                </div>
                  ...
                  <div>
                    ...
                    <Select onValueChange={(value) => {
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="photo-friendly">Photo Friendly</SelectItem>
                        <SelectItem value="bring-friends">Bring Friends</SelectItem>
