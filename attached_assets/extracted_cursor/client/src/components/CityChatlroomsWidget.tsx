...
export function CityChatlroomsWidget({ city, state, country }: CityChatlroomsWidgetProps) {
  ...
  const handleJoinChatroom = (chatroomId: number, chatroomName: string) => {
    // Navigate to city-specific chatrooms page with proper city formatting
    const fullCityName = state && country ? `${city}, ${state}, ${country}` : 
                        country ? `${city}, ${country}` : city;
    window.location.href = `/city-chatrooms/${encodeURIComponent(fullCityName)}`;
  };
