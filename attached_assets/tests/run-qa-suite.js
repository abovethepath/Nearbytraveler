...
const runTests = async () => {
  ...
  for (const category of testCategories) {
    ...
    try {
      ...
      console.log(`⚠️  Tests in ${category.name} had issues`);
      results.push({
        category: category.name,
        passed: 0,
        failed: 1,
  ...
  if (criticalFailures.length > 0) {
    ...
    criticalFailures.forEach(f => {
      console.log(`   • ${f.category}: ${f.failed} failures`);
    });
    console.log('\n   ⚠️  Deploy NOT recommended until critical issues are fixed');
  } else {
    console.log('\n✨ All critical tests passing - Ready for deployment!');
  }
  
  console.log(`\n📊 Full report saved to: qa-test-report.md`);
};
...
${results.filter(r => r.critical && r.status !== 'PASS').length === 0 
  ...
  : '❌ **NOT RECOMMENDED** - Critical test failures detected'
}
