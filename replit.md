...
- **City Page Tagline Removed (Aug 2025)**: Removed "Your premier destination for authentic local experiences" tagline from city page headers per user request for cleaner design.
- **Home Page Animations Removed (Aug 2025)**: Removed all moving animations including floating text, scaling buttons, and slide-in effects from home page per user request to eliminate distracting movement. Kept only subtle color transitions for better user experience.
- **Widget Repositioning (Aug 2025)**: Moved SecretExperiencesWidget higher up in city page sidebar layout - now positioned after CityStatsWidget and before travel tips/chatroom sections for better visibility.
...
- **Username Length Constraints (Aug 2025)**: Implemented 6-14 character limit for all usernames with both frontend validation (Zod schema) and database constraints (varchar(14) + check constraint). Username "nearbytraveler" (14 characters) now fits within the updated limit.
...
- **Fixed Los Angeles Metro City Stats & Events (Aug 2025)**: Fixed critical issues where Los Angeles Metro showed zero stats and no events/activities. Created proper city page record in database, linked secret experiences to correct city_page_id, and enabled metro consolidation in events API to search all LA area cities (Santa Monica, Playa del Rey, Hollywood, etc.). City stats now use proper metro area user counting.
- **Fixed Metro Area Secret Activities System (Aug 2025)**: Completely resolved critical regression where ALL metropolitan area secret activities stopped displaying. Fixed logic for both Los Angeles Metro and Nashville Metro to properly search all constituent cities when users visit metro area pages. Los Angeles Metro now shows secret activities from Culver City and Playa del Rey users, Nashville Metro shows activities from Nashville users. Metropolitan consolidation system now works bidirectionally - individual city users contribute to metro area displays.
- **Family Travel Checkbox Fixed (Aug 2025)**: Resolved critical "Traveling with children" checkbox issue where database boolean values weren't properly converting to frontend form values. Fixed React Hook Form initialization with proper boolean conversion using `!!(user.travelingWithChildren)` and force setValue after form reset. Checkbox now correctly displays checked state when database contains `true`, saves/loads properly, and automatically clears children ages field when unchecked.
- **SMS Event Notifications System (Aug 2025)**: Implemented complete SMS notification system for event RSVPs using Twilio integration. Added phone number collection to signup forms with optional field explaining SMS event notifications. Created SMSService with RSVP confirmations, event reminders, cancellations, and updates. Integrated with event join endpoint to automatically send confirmation texts when users RSVP. Phone numbers stored in existing database schema, with proper validation and US/international number formatting.
- **Landing Page Content Reorganization (Aug 2025)**: Streamlined locals and events landing pages by moving Venice Beach Dance Party and other event-focused experiences to events page, while keeping community connection features on locals page. Eliminated redundant feature sections - kept unique "What Makes Nearby Traveler Special" cards (Beach Bonfires, Local Connections, Host Events) while removing duplicates. Updated feature card colors to match exact design specifications with vibrant dark backgrounds and white text.
- **Vibrant Landing Page Design Overhaul (Aug 2025)**: Implemented high-energy, colorful branding across all landing pages with orange/teal/blue color schemes. Added dynamic taglines with "right now, today!!!" messaging. Updated locals page hero to "Be the local you'd want to meet" with clean subhead about sharing hidden gems and connecting with travelers and locals.
