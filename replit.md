...
- **Fixed Nashville Metro Secret Activities Bug (Aug 2025)**: Resolved critical regression where Nashville Metro secret activities stopped displaying after username changes. Issue was caused by missing "Nashville Metro" entry in metropolitan area consolidation arrays. Added "Nashville Metro" to both consolidation functions so API properly converts "Nashville Metro" → "Nashville" for database queries. Secret activities from nearbytravl30 ("TOOTSIES AND THE STAGE") now display correctly on Nashville Metro city pages.
