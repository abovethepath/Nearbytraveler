...
MEETUP SYSTEMS — PHASE 1 CLEANUP COMPLETE: The dead `hangouts`/`hangout_participants` tables and `LocalHangouts.tsx` component have been removed (they had no routes and no data). The deprecated `/api/quick-meetups/*` route block (898 lines) has been deleted; all endpoints now live exclusively under `/api/quick-meets/*`. Frontend calls in `MeetupAlertBanner.tsx` and `TripItineraryView.tsx` updated to use `/api/quick-meets`. The two live systems are: (1) Available Now (`available_now` + `available_now_requests` tables, `AvailableNowWidget.tsx`, 1:1 request-gated model) and (2) Quick Meets (`quick_meetups` + `quick_meetup_participants` tables, `QuickMeetupWidget.tsx`, open-join model). Phase 2 is unification of these two into a single `meetup_sessions` table.
