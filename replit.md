...
- **Business Profile Display**: Clean hero section with business name prominent at top, "Nearby Business" badge, clickable phone/website in professional grid layout. Military status only displays once in Business Information section for veteran/active duty owned businesses. Business description shows in About section without redundant name display.
...
- **Streamlined Business Signup (August 2025)**: Created simplified business signup flow collecting only essential information (business name, type, phone, location, optional website and GPS coordinates). Comprehensive profile completion emphasized post-registration with prominent messaging about requirement to complete bio, interests, activities, and languages for proper user matching. All advanced business features (diversity ownership, detailed contact info, extensive business details) moved to profile editing phase.
...
- **Custom Text Entry System**: Implemented comprehensive custom text input capabilities allowing all users to enter personalized interests, activities, and events beyond predefined options. Added custom_interests, custom_activities, and custom_events database fields with comma-separated format support. Available in both travel plan editing and main profile editing forms for all user types (travelers, locals, business users). Custom entries automatically sync to main arrays for seamless integration. Fixed critical issues with custom field initialization, form submission on Enter key, and proper separation of predefined vs custom entries in business profiles.
- **Business Profile Editing Overhaul (August 2025)**: Comprehensive cleanup and consolidation of business profile editing interface. Removed duplicate "Business Interests & Activities" sections. Consolidated duplicate business location forms into single unified form with street address, ZIP code, and standard country/state/city dropdowns. Fixed languagesSpoken field mapping issue (camelCase to snake_case) that prevented language preferences from saving. Restored main profile "Edit Profile" button that was accidentally removed. Enhanced dark mode styling for military status checkboxes. Business users now have clean, non-duplicated editing experience with proper data persistence.
- **Diversity Business Ownership Categories (August 2025)**: Added comprehensive diversity ownership categorization system for businesses including Minority Owned, Female Owned, and LGBTQIA+ Owned Business options. Each category features privacy controls allowing businesses to hide public display while maintaining keyword searchability for diversity initiatives. Categories display with distinct colored styling and comprehensive privacy disclaimers. Integrated into business profile editing forms with proper database field mapping and form validation.
- **Languages Widget Priority Repositioning (August 2025)**: Moved Languages widget to top-right position in profile layout for enhanced customer visibility importance. Languages now appear as the first widget in the right sidebar with prominent blue gradient styling, enhanced border treatment, and improved visual hierarchy. This repositioning reflects the critical importance of language information for customer interactions and business communications.
- **Business Geolocation Mapping System (August 2025)**: Implemented comprehensive geolocation functionality for businesses with active deals/events. Created dedicated API endpoint `/api/businesses/map` that filters businesses requiring both GPS coordinates AND active deals/events to appear on city maps. Updated CityMap component to use dual data sources - regular city data and business geolocation data. Added map disclaimer indicating limitations without accountability for functionality issues. System ensures only businesses with current offerings appear on maps for relevant customer discovery.
- **Enhanced Business Contact Management (August 2025)**: Fixed critical business signup flow by restoring required street address field for location services. Added separate owner contact information (ownerName, ownerPhone) distinct from business customer contact info. Owner details enable platform communication while business phone serves customers. Enhanced quick deals API to include comprehensive business information (name, street address, city, phone) for customer-facing deal cards. All business location data now transfers properly to profiles and appears prominently in deal discovery.
- **Business Admin Contact Section (August 2025)**: Created dedicated admin section in business profiles for owner contact information that's hidden from public view. Only business owners can see and edit their internal contact details (owner name, owner phone) through a private admin card with purple styling and Shield icon. Information is clearly marked as internal-use-only for platform communications, ensuring separation between public business contact info and private owner details.
- **Email Variant Support for Multiple Businesses (August 2025)**: Implemented comprehensive email variant system allowing business owners to manage multiple businesses using one email inbox. Added guidance in business signup forms and welcome page explaining email variants (e.g., owner@example.com, owner+restaurant@example.com, owner+shop@example.com). Enhanced form validation to accept email variants while maintaining unique email constraints. Includes helpful tooltips and examples throughout the signup process. Business owners can now easily manage multiple business types (restaurant, retail, services) under one email account while maintaining separate business profiles, deals, and customer interactions.
