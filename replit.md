...
- **TRAVEL MEMORY ALBUMS WITH OPTIONAL DATES COMPLETED** (August 11): CRITICAL SUCCESS - Fully implemented optional startDate and endDate timestamp fields for travel memory albums. Added database schema updates, frontend form enhancements with trip duration inputs, backend API support for date handling, and edit form functionality. Users can now create travel albums and optionally specify trip start/end dates for better duration tracking.
- **LANDING PAGE HERO IMAGE REVERTED** (August 11): CRITICAL SUCCESS - Reverted landing page back to single static hero image instead of scrolling gallery per user preference. Landing page now displays one fixed image as originally designed.
...
- **WELCOME MESSAGE FEATURE ENHANCEMENT** (August 11): CRITICAL SUCCESS - Created comprehensive welcome messages with bullet points listing all website features. Separate optimized versions for regular users (12 key features) and businesses (10 business-focused features) replacing previous @nearbytraveler personal message format.
- **PEOPLE DISCOVERY WIDGET TEXT FIXED** (August 11): CRITICAL SUCCESS - Fixed compatibility display in People Discovery Widget to show "What you have in common: [number]" instead of "Discovering compatibility...". Corrected API endpoint to properly fetch compatibility data from server.
- **QUICK MEETUP PROFILE FILTERING FIXED** (August 11): CRITICAL SUCCESS - Fixed profile pages to only show user's own quick meetups instead of displaying other users' meetups. Added userId parameter filtering to QuickMeetupWidget and server API.
- **SIGNUP DROPDOWN TEXT CORRECTED** (August 11): CRITICAL SUCCESS - Changed travel signup dropdown text from "select current city" to "select travel city" for better clarity during registration process.
...
- **RECURRING EVENT DISPLAY FIXED** (August 11): CRITICAL SUCCESS - Added recurring event information display to event details page. Events marked as recurring now show "Repeats weekly/monthly/daily until [end date]" in the event details with calendar icon.
- **EVENT TIME CREATION FIXED** (August 11): CRITICAL SUCCESS - Fixed event creation route that was double-processing date conversion, causing incorrect event times. Events now preserve the exact time selected by user during creation. Removed redundant date conversion that was altering timestamps.
- **EVENT ORDERING FIXED** (August 11): CRITICAL SUCCESS - Fixed all event listing endpoints to show newly created events first. Changed ordering from event date to creation order (ID descending) across getAllEvents, getUpcomingEvents, getEventsByLocation, getEventsByMetropolitanArea, getUserEvents, and getEventsByOrganizer methods.
