...
- **Database Connection Stability Fix (Aug 12, 2025)**: Resolved critical Neon serverless WebSocket connection errors that were causing uncaught exceptions and app crashes. Replaced `@neondatabase/serverless` with more stable `postgres` library using direct PostgreSQL connections. This eliminated the "Cannot set property message of #<ErrorEvent> which has only a getter" error and restored app functionality.
