 ...
 */
export function detectMetroArea(city) {
    const cityLower = city.toLowerCase().trim();
    for (const [metroName, cities] of Object.entries(METRO_AREAS)) {
        const isInMetro = cities.some(metroCity => cityLower === metroCity.toLowerCase() ||
 ...
 */
export function getMetroAreaName(city) {
 ...
 */
export function getMetroCities(metroAreaName) {
    return METRO_AREAS[metroAreaName] || [];
}
/**
 * Check if two cities are in the same metro area
 */
export function areInSameMetro(city1, city2) {
