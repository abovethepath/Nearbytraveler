...
export function isLAMetroCity(city) {
    if (!city)
        return false;
    const normalizedCity = city.toLowerCase().trim();
    const laMetro = METRO_AREAS['Los Angeles'];
    return laMetro.cities.some(metroCity => normalizedCity.includes(metroCity.toLowerCase()));
}
// Helper function to get metro area for a city
export function getMetroArea(city) {
    if (!city)
        return null;
    const normalizedCity = city.toLowerCase().trim();
    for (const [key, metro] of Object.entries(METRO_AREAS)) {
        if (metro.cities.some(metroCity => normalizedCity.includes(metroCity.toLowerCase()))) {
...
export function getMetroCities(metroOrCity) {
