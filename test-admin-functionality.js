...
async function testGetMembers(chatroomId) {
  ...
  try {
    ...
    logTest(`Get members for chatroom ${chatroomId}`, success, 
      success ? `Found ${response.data.length} members` : `Status: ${response.statusCode}`);
...
async function testPermissionDenied(action, chatroomId, targetUserId) {
  console.log(`\n🚫 Testing Permission Denied: ${action}...`);
  
  const result = await testAdminAction(action, chatroomId, targetUserId, 403);
  return result.success; // Success means we got the expected 403
}

async function testUnauthorizedAccess(chatroomId) {
  ...
  try {
    ...
    logTest('Unauthorized access protection', unauthorized, 
...
async function runTests() {
  ...
  try {
    ...
    await testUnauthorizedAccess(TEST_CONFIG.chatrooms.adminTest);
    ...
    if (getMembersSuccess && members.length > 0) {
      ...
      console.log(`\nFound test subjects:`);
      console.log(`- Admin: ${adminMember ? adminMember.name : 'None'}`);
      console.log(`- Member: ${regularMember ? regularMember.name : 'None'}`);
      console.log(`- Owner: ${ownerMember ? ownerMember.name : 'None'}`);
      ...
      if (regularMember) {
        // Test valid admin actions
        await testAdminAction('promote', TEST_CONFIG.chatrooms.adminTest, regularMember.userId, 200);
        await testAdminAction('demote', TEST_CONFIG.chatrooms.adminTest, regularMember.userId, 200);
        await testAdminAction('remove', TEST_CONFIG.chatrooms.adminTest, regularMember.userId, 200);
      }
      ...
      // Test self-action protection
      const currentUserId = TEST_CONFIG.users.admin.id;
      await testPermissionDenied('remove', TEST_CONFIG.chatrooms.adminTest, currentUserId);
    }
    
    // Test 5: Different Chatroom Tests
    console.log('\n📋 SECTION 5: MULTIPLE CHATROOM TESTS');
    console.log('-'.repeat(50));
